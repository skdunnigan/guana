...
dat_nut_corr <- dat_nut %>%
  filter(sitetype == "OpenWater" & WBID == "River") %>%
...
ggcorrplot(corr,
  ...
  labs(title = 'Correlation Plot 1, open water river sites',
       caption = 'Spearman Rank correlations, complete obs',
       subtitle = 'basic')
ggsave("output/correlations/openwater_1_river.png", width = 7.5, height = 7.5, dpi = 300)
...
ggcorrplot(corr, p.mat = p.mat,
  ...
  theme(# everything in theme is strictly aesthetics
    ...
    plot.title = element_text(size = 16, face='bold'),
    plot.subtitle = element_text(size = 11, face = 'italic')) +
  labs(title = 'Correlation Plot 2, open water river sites',
       caption = 'Spearman Rank correlations, complete obs',
       subtitle = 'insignificant correlations removed and theme changed')
ggsave("output/correlations/openwater_2_river.png", width = 7.5, height = 7.5, dpi = 300)
...
ggcorrplot(corr, p.mat = p.mat,
  ...
  theme(# everything in theme is strictly aesthetics
    ...
    plot.title = element_text(size = 16, face='bold'),
    plot.subtitle = element_text(size = 11, face = 'italic')) +
  labs(title = 'Correlation Plot 3, open water river sites',
       caption = 'Spearman Rank correlations, complete obs',
       subtitle = 'insignificant correlations removed and correlation coefficients')
ggsave("output/correlations/openwater_3_river.png", width = 8, height = 8, dpi = 300)
